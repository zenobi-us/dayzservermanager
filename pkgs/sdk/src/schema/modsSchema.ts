...
export const EPublishedFileQueryTypeSchema = z
  .enum([
  ...
  .pipe(z.number().int().min(0).max(19, 'query_type must be between 0 and 19'));

export const PublishedFileServiceQueryFilesRequestParamsSchema = z
  .object({
    ...
    child_publishedfileid: z
      .string()
      .regex(/^\d+$/, 'child_publishedfileid must be a numeric string')
      .optional(),
    ...
    desired_revision: z
      ...
      .int()
      .min(0)
      .max(1, 'desired_revision must be 0 or 1')
      .optional(),
    revision: z
      .number()
      .int()
      .min(0)
      .max(2, 'revision must be between 0 and 2')
      .optional(),
  })
  .strict();

export type IPublishedFileServiceQueryFilesRequestParams = z.infer<
  typeof PublishedFileServiceQueryFilesRequestParamsSchema
>;

// Schema for individual tag objects
export const SteamWorkshopTagSchema = z.object({
  tag: z.string(),
  display_name: z.string(),
});

// Main schema for the Steam Workshop search result
export const SteamWorkshopSearchResultItemSchema = z.object({
...
export const SteamWorkshopSearchResultsSchema = z.object({
  response: z.object({
    total: z.number(),
...
// Type definitions
export type SteamWorkshopTag = z.infer<typeof SteamWorkshopTagSchema>;
export type SteamWorkshopSearchResultItem = z.infer<
  typeof SteamWorkshopSearchResultItemSchema
>;
export type SteamWorkshopSearchResults = z.infer<
  typeof SteamWorkshopSearchResultsSchema
>;

export const CustomXmlItemSchema = z.object({
  name: z.string(),
});
export type CustomXmlItem = z.infer<typeof CustomXmlItemSchema>;
...
export const ModItemListSchema = z.array(ModItemSchema);
export type ModItemList = z.infer<typeof ModItemListSchema>;

export const ModItemDetailSchema = z
  .object({
    size: z.number(),
    customXML: z.array(CustomXmlItemSchema),
  })
  .merge(ModItemSchema);
